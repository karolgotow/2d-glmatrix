...
#define DEF_TEXT     "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua."
...
#define GRID_SIZE  77     /* width and height of the arena */
#define GRID_DEPTH 5     /* depth of the arena */
#define WAVE_SIZE  66     /* periodicity of color (brightness) waves */
...
static argtype vars[] = {
 ...
 {&do_text,    "text",    "Text", DEF_TEXT,   t_String},
...
reset_strip (ModeInfo *mi, strip *s)
{
 ...
 /* char *fgets_return;*/
  ...
  s->x = (GLfloat) (floor(frand(GRID_SIZE*4)) - (GRID_SIZE*2));
  s->y = (GLfloat) (GRID_SIZE/2 + BELLRAND(0.15));      /* shift top slightly */
  ...
  for (i = 0; i < mp->nstrips; i++)
    {
      strip *x_s = &mp->strips[i];
     	if( s->x == x_s->x && ! (x_s->y == s->y) && ! (x_s->y == GRID_SIZE-5) && x_s->brightness > s->brightness / 2 )
			{
				s->brightness = 0.005 + BELLRAND(0.1);
				/*s->y = GRID_SIZE-5;*/
			}
    }
...
/*s->brightness -= 0.000005;*/
 /* if (s->spinner_y >= GRID_SIZE * 0.9 )*/
  /* {*/
	 /*s->wave_position = 0;*/
	/* s->wave_speed = 0;*/
	/*if(s->brightness < 0.41 && s->cleaner_y >= GRID_SIZE)*/
		/*{
			s->brightness -= frand(0.006) * speed;
		}
    }*/

  if (s->spinner_y >= GRID_SIZE*4)
    {
	 s->spinner_y = GRID_SIZE*4;
	/* s->spinner_speed = 0;*/
         s->cleaner_y += s->spinner_speed;
	 if(s->cleaner_y > GRID_SIZE){s->cleaner_y = GRID_SIZE;}
	 array_int = round(s->cleaner_y);
	 s->glyphs[ array_int ] = 1;
...
reshape_matrix (ModeInfo *mi, int width, int height)
{
  ...
  glLoadIdentity();
  gluPerspective (112.0, 1/h/0.80, 1.0, 100); /* 40 / 0.25*/
...
init_matrix (ModeInfo *mi)
{
  ...
  if(text_file != NULL)
	{
	int fgetc_char = 0;
	while(fgetc_char != EOF)
		{
			 fgetc_char = fgetc(text_file);
   			 if(fgetc_char == '\n')
      			 text_file_lines++;
		}
	rewind(text_file);
